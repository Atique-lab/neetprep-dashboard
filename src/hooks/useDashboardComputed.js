...
export function useDashboardComputed(filteredData, rawData, extraData, loading, error) {
  return useMemo(() => {
    ...
    const lsMonthlyStudentsByCentre = {};
    ...
    lastSessionProcessedDeduplicated.forEach(d => {
      ...
      const month = getMonth(d.date);
      if (month) {
        if (!lsMonthlyStudentsByCentre[ctr]) lsMonthlyStudentsByCentre[ctr] = {};
        lsMonthlyStudentsByCentre[ctr][month] = (lsMonthlyStudentsByCentre[ctr][month] || 0) + 1;
      }
    });
    ...
    const lastSessionComparison = {
      ...
      monthlyStudentsByCentre: lsMonthlyStudentsByCentre,
    };
  ...
  }, [filteredData, rawData, extraData, loading, error]);
}
